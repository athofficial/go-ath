...
func init() {
	// Run the app if we're the child process for rungath.
...
func rungath(t *testing.T, args ...string) *testgath {
	tt := &testgath{T: t, Executable: os.Args[0]}
	...
	// Boot "gath". This actually runs the test binary but the init function
...
func (tt *testgath) InputLine(s string) string {
	io.WriteString(tt.stdin, s+"\n")
	return ""
}

func (tt *testgath) setTemplateFunc(name string, fn interface{}) {
	if tt.Func == nil {
...
// before matching.
func (tt *testgath) expect(tplsource string) {
...
func (tt *testgath) matchExactOutput(want []byte) error {
...
// after expectRegexp.
func (tt *testgath) expectRegexp(resource string) (*regexp.Regexp, []string) {
	var (
...
// returned.
func (tt *testgath) stderrText() string {
	tt.stderr.mu.Lock()
	defer tt.stderr.mu.Unlock()
	return tt.stderr.buf.String()
}

func (tt *testgath) withKillTimeout(fn func()) {
