...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 30696, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 30696, peersTotal: 50, peersLight: 0, gasTarget: 7.5, gasLimit: 10, gasPrice: 1}
		}
	}
	existed := err == nil
	...
	if w.conf.Genesis.Config.Ubqhash != nil && !boot {
		fmt.Println()
		if infos.ubqhashdir == "" {
			fmt.Printf("Where should the ubqhash mining DAGs be stored on the remote machine?\n")
			infos.ubqhashdir = w.readString()
		} else {
			fmt.Printf("Where should the ubqhash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ubqhashdir)
			infos.ubqhashdir = w.readDefaultString(infos.ubqhashdir)
		}
	}
