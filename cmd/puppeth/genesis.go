...
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ubqhash is currently supported between go-ethereum and aleth
	if genesis.Config.Ubqhash == nil {
...
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ubqhash is currently supported between go-ethereum and Parity
	if genesis.Config.Ubqhash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.Ubqhash.Params.BlockReward = make(map[string]string)
	spec.Engine.Ubqhash.Params.DifficultyBombDelays = make(map[string]string)
	// Frontier
...
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only ubqhash is currently supported between go-ethereum and pyethereum
	if genesis.Config.Ubqhash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
